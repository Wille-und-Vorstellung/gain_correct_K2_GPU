...
//read the pixel value of (x,y,z) x,y,z counted from 0;
int mrc_read_pixel(float *pix_gray, FILE *fin, MrcHeader  *head, int x, int y, int z );
...
int mrc_read_slice_byte(FILE *fin, MrcHeader  *head, int slcN, char axis, unsigned char *slcdata);

//read all the data to mrc_data_all;
...
//generate a new head of mrc file and initiate it;
...
int mrc_add_sliceN_byte(FILE *fout, MrcHeader  *headout, unsigned char *slcdata, int slcN);
...
//updata slcX , slcN is the Nth number of the slice, starting from 0;
