...
int dispatcher_gpu(float *coord_l, float *gain_l , long size_x, long size_y, long slice_n, int type){
	...
	cudaMemcpy( device_coord, coord_l, size_x*size_y*slice_n, cudaMemcpyHostToDevice );
	//cudaMemcpy( device_gain, gain_l, size_x*size_y, cudaMemcpyHostToDevice );

	//activate kerneld
	switch(type){
		case 0://c
			kernel_type_c<< GRID_BLOCK, BLOCK_SIZE >>( device_coord, device_gain, size_x, size_y, slice_n );
			break;
		case 1://s
			kernel_type_su<< GRID_BLOCK, BLOCK_SIZE >>( device_coord, device_gain, size_xsize_y, slice_n );			
			break;
		case 2://f
			kernel_type_f<< GRID_BLOCK, BLOCK_SIZE >>( device_coord, device_gain, size_xsize_y, slice_n );
			break;
		case 6://u
			kernel_type_su<< GRID_BLOCK, BLOCK_SIZE >>( device_coord, device_gain, size_xsize_y, slice_n );			
			break;
...
void mutiplier_kernel_type_c(float *coord, float *gain, long size_x, long size_y, long slice_n){
	...
	for (int i=0; i<unit_n; i++){
		...
		coord[index*unit_n+i] = (*(unsigned char*)coord[index*unit_n+i])*gain[slice_c];
	}
	//__threadfence()
	return;
}
void mutiplier_kernel_type_su(float *coord, float *gain, long size_x, long size_y, long slice_n){//execute all those mutiplications of type_s data
	//execute all those mutiplications of type_c data
	long index=0; 
	//long unit_n = size_x*size_y*slice_n/GRID_BLOCK;
	long unit_n = UNIT_N;
	...
	for (int i=0; i<unit_n; i++){
		...
		slice_c = (index*unit_n+i)%(size_x*size_y);
		coord[index*unit_n+i] = (*(short *)coord[index*unit_n+i])*gain[slice_c];
	}
	//__threadfence()
	return;
}
void mutiplier_kernel_tpye_f(float *coord, float *gain, long size_x, long size_y, long slice_n){//execute all those mutiplications of type_f data
	//execute all those mutiplications of type_c data
	long index=0; 
	//long unit_n = size_x*size_y*slice_n/GRID_BLOCK;
	long unit_n = UNIT_N;
	...
	for (int i=0; i<unit_n; i++){
		...
		slice_c = (index*unit_n+i)%(size_x*size_y);
		coord[index*unit_n+i] = (*(float *)coord[index*unit_n+i])*gain[slice_c];
	}
	//__threadfence()
	return;
}
