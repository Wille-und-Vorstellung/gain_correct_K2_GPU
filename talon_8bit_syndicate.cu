...
//syndicate_IV, 'transfer kernel' removed
...
#define SIZE 2000 //debug only, alpha_test(), do not touch
#define B_SIZE 28125 // debug only, beta_test(), do not touch
#define B_GAIN_S 5625//do not touch
...
int defect_gain_correct(char *fin, char *gain, char *fout, MrcHeader *head,int threads){
	...
	float indicator=0;
	indicator=dispatcher_gpu_8bit_syndicate( coor_xy, source, gain_xy, size_x, size_y, slice_n, file_type );
...
float dispatcher_gpu_8bit_syndicate( char *coord_l, void *src,float *gain_l , long size_x, long size_y, long slice_n, int type /*, long src_size */){
	...
	if ( type != 0 && type != 1 && type != 2 && type != 6 ) {
		printf( "dispatcher_gpu_8bit_syndicate: wrong input Mrc type: %d \n", type );
		return -1;
	}
	int step=0;
	long offset_temp=0;
	...
	int left_over = 0;
	cudaError_t f1, f2, f3, f4, f5, f6, f7, f8, f9, f10;
	...
	printf("Initialize allocation & memcpy.\n");
	cudaSetDevice( GPU_DEVICE_0 );
	f1 = cudaMalloc( (void **)&device_coord_1,  sizeof(char)*total_s/2  );
	f2 = cudaMalloc( (void **)&device_src_1, step*(total_s/2) );
	f3 = cudaMalloc( (void **)&device_gain_1,  sizeof(float)*single_s );
	f4 = cudaMemcpy( device_src_1, src, step*(total_s/2), cudaMemcpyHostToDevice );
	f5 = cudaMemcpy( device_gain_1, gain_l, sizeof(float)*single_s, cudaMemcpyHostToDevice );
	cudaSetDevice( GPU_DEVICE_1 );
	f6 = cudaMalloc( (void **)&device_coord_2,  sizeof(char)*(total_s/2 + left_over) );
	f7 = cudaMalloc( (void **)&device_src_2, step*(total_s/2 + left_over) );
	f8 = cudaMalloc( (void **)&device_gain_2,  sizeof(float)*single_s );
	f9 = cudaMemcpy( device_src_2, (void*)((char*)src + offset*step), step*(total_s/2 + left_over), cudaMemcpyHostToDevice );
	f10 = cudaMemcpy( device_gain_2, gain_l, sizeof(float)*single_s, cudaMemcpyHostToDevice );
	//GRAM allocation check
	if ( f1 != cudaSuccess || f2 != cudaSuccess || f3 != cudaSuccess || f4 != cudaSuccess || f5 != cudaSuccess || f6 != cudaSuccess || f7 != cudaSuccess || f8 != cudaSuccess || f9 != cudaSuccess || f10 != cudaSuccess ){
		printf( "cuda memory allocation failed\n" );
		exit(4);
	}
	printf("Allocation & memcpy done.\n");
	...
	printf("Insider: the unit_n is %d \n", unit_n);	
			...
			cudaSetDevice( GPU_DEVICE_0 );
			multiplier_kernel_8bit_syndicate_unitype<<< GRID_BLOCK, BLOCK_SIZE >>>( (char*)device_coord_1, (float*)device_gain_1, (char*)device_src_1, total_s/2, single_s, unit_n, 0, step ); 
			//HAZARD: slice_c might be a problem... + a offset, yes offset will do.BUT NOT FULLY VERIFIED YET
			cudaSetDevice( GPU_DEVICE_1 );
			multiplier_kernel_8bit_syndicate_unitype<<< GRID_BLOCK, BLOCK_SIZE >>>( (char*)device_coord_2, (float*)device_gain_2, (char*)device_src_2, total_s/2+left_over, single_s, unit_n, offset, step );
			printf("Serpent out\n");
...
__global__ void multiplier_kernel_8bit_syndicate_unitype(char *coord, float *gain, char* src, long long total_s, long long single_s, long long unit_n, long long offset, int step){
	//execute mutiplications regardless of src type
	long long index=0;
	long long i = 0; 
	...
	for (i=0; i<unit_n; i++){
		...
		coord[index*unit_n+i] =  (char)(((float)*(src + (index*unit_n+i)*step)) * (float)gain[slice_c]);
	}	
	return;
}
...
bool alpha_test(){
	...
	/*
	float dispatcher_gpu_8bit_syndicate(float *coord_l, void *src, float *gain_l , long size_x, long size_y, long slice_n, int type, long );
	*/
	dispatcher_gpu_8bit_syndicate( des, source, gain, SIZE/(2*x_size), x_size, 2, 2/*,SIZE*4*/ );
...
bool beta_test( int type ){
	...
	float dispatcher_gpu_8bit_syndicate(float *coord_l, void *src, float *gain_l , long size_x, long size_y, long slice_n, int type, long );
	*/
	dispatcher_gpu_8bit_syndicate( des, source, gain, 75, 75, 5, type/*, 75*75*5*sizeof(char)*/ );
